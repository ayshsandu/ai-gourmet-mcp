...
const MENU_ITEMS = JSON.parse(readFileSync(MENU_JSON_PATH, "utf-8"));
// In-memory storage
const ORDERS = {};
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    switch (name) {
        ...
        case "get_item_details": {
            const { item_identifier } = args;
            const item = MENU_ITEMS.find(item => item.id === item_identifier || item.name.toLowerCase() === item_identifier.toLowerCase());
        ...
        case "find_items_by_criteria": {
            const { dietary_preference, max_price, exclude_allergens, category } = args;
            ...
            if (exclude_allergens) {
                for (const allergen of exclude_allergens) {
                    filtered = filtered.filter(i => !i.allergens.some(a => a.toLowerCase() === allergen.toLowerCase()));
                }
            }
            if (category) {
        ...
        case "add_to_cart": {
            const { session_id, item_id, quantity = 1 } = args;
        ...
        case "checkout": {
            const { session_id, customer_info, payment_info } = args;
            const session = ACTIVE_SESSIONS[session_id];
            if (!session) {
                return {
                    content: [
